...
Watchdog_service::handle_zmq(){
	try
	{
		...
		//watchdog::pb_watchdog_message test;
		//watchdog::pbMessage pb_msg;

		//Check for new messages
		if(!zmq_service.receive(&pb_msg, ZMQ_NOBLOCK)) return; // No message to handle
		
		//Got a message to handle

		printf("Received: %s\n", pb_msg.SerializeAsString().c_str());
...
Watchdog_service::handle_message(ladybug5_network::pb_start_msg &message){
	...
	reply_msg.set_allocated_start(&message);
	...
	if(path.empty()){ 
		...
		std::string key = config.get_path(key_autostart);
